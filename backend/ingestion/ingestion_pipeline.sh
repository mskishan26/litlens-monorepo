...
#SBATCH --nodes=1
#SBATCH --gres=gpu:a100:1                           # Accept any GPU (A100 or H200)
#SBATCH --time=02:00:00                             # Job runtime   (needs to be updated, need to find the approx time for a file and then change this)
#SBATCH --job-name=ingestion_pipeline               # Job name
#SBATCH --mem=16GB                                  # Job memory
#SBATCH --ntasks=1                                  # Number of tasks
#SBATCH --cpus-per-task=8                           # Number of cpus per task
#SBATCH --output=ingestion_pipeline_%j.out          # Output file
#SBATCH --error=ingestion_pipeline_%j.err           # Error file
...
# Data folder paths
export BASE_DIR="/scratch/sathishbabu.ki/data_files1"
...
source activate /scratch/sathishbabu.ki/processing_marker
...
conda deactivate
source activate /scratch/sathishbabu.ki/processing_vllm
